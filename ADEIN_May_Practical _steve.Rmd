...
Experiment with Kenya, to change different country codes, check the 3 letter country code for that country, and replace the first line in the section below with it
```{r}
mydata_kenya = data.frame(COUNTRY="KEN")
...
for (i in indicators$ID) {
  ...
  k=match(i,indicators$ID)
  label(xx[[i]]) <- indicators[k,1]
  mydata <- full_join(mydata_kenya, xx)
}
